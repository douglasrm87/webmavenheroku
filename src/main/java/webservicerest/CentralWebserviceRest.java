...
public class CentralWebserviceRest {
	...
	private final static String[] urls = { "http://localhost:8080/webmavenheroku",
			...
			"http://192.168.15.17:8080/webmavenheroku/rest/centralservicos/1/123456/Douglas" };
	...
	// http://localhost:8080/webmavenheroku/rest/centralservicos/123456
	@GET
	@Path("/{cpf}")
	public Response obterConsumo(@PathParam("cpf") int cpf) {
		for (ClienteDTO cliente : EscopoAplicacao.listaClienteCartaoFidelidade) {
			if (cpf == cliente.getCpfCliente()) {
				ResponseBuilder response = Response.status(200).entity("Consulta ao consumo do CPF:"
						+ cliente.getCpfCliente());
				/* + ":" + cliente.obterBonusCartaoFidelidade())*/
				return response.build();
			}
		}
		ResponseBuilder response = Response.status(200).entity("CPF nao localizado.");
		return response.build();
	}
